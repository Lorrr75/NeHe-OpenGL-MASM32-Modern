...
; Definizione della macro testuale pura
...
.data?
hInst	DD ?			; variabile per la mmorizzazione dell'istanza del programma
hRC	DWORD ?
hDC	HDC ?
hWnd	HWND ?
...
Titolo		db "NeHe's Color Tutorial",0
...
Si_Fullscreen:
	xor	eax, eax
	; l'azzeramento di eax, serve per inserire un valore considerato Byte in una DWord
	mov	al, fullscreen
	...
	;invoke	CreateGLWindow, addr Titolo, 640, 480, 16, eax		; il tutorial prevedeva 16 bit di colore
	invoke	CreateGLWindow, addr Titolo, 640, 480, 32, eax		; il mio monitor a schermo intero restituisce errore
...
RegisterClass_OK:
	...
	;mov	eax,  DM_BITSPERPEL or DM_PELSWIDTH or DM_PELSHEIGHT
	...
	; l'istruzione qui sopra a me non funziona bisogna usare lea eax e la call
;	push	CDS_FULLSCREEN				; salva dwflags (valore 4)
;	lea	eax, dword ptr dmScreenSettings		; carica offset dmScreenSettings
;	push	eax					; salva sullo stack
;	call	ChangeDisplaySettings			; cambia modalità a chermo intero
...
AdjustWindow_Rect:
	...
	mov	ebx, WS_CLIPSIBLINGS or WS_CLIPCHILDREN
...
No_Fix_0_DIVIDE:
	...
	; ora che abbiamo le variabili possiamo invokare la funzione OpenGL
	; ho deciso di utilizzare l'assembly a 32 bit, per i 64 bit sono ancora il fase iniziale di studio
	mov	eax, DWORD ptr [zFar+4]			; per ogni valore salviamo sullo stack prima il valore 32 bit basso 
...
DrawGLScene	proc
	...
	MpushReal4 0.0
	MpushReal4 -1.5
	call	glTranslatef				; sposta a sinistra e in dentro			
	
	invoke	glBegin, GL_TRIANGLES			; Disegnare usando i triangoli	

		MpushReal4 0.0				; componente Blu				
		MpushReal4 0.0				; componente Verde
		MpushReal4 1.0				; componente Rosso
		call	glColor3f			; usa colore per disegnare
				
		MpushReal4 0.0
		MpushReal4 1.0
		MpushReal4 0.0
		call	glVertex3f			; Cima

		MpushReal4 0.0				; componente Blu				
		MpushReal4 1.0				; componente Verde
		MpushReal4 0.0				; componente Rosso
		call	glColor3f			; usa colore per disegnare

		MpushReal4 0.0
		MpushReal4 -1.0
		MpushReal4 -1.0
		call	glVertex3f			; In basso a sinistra

		MpushReal4 1.0				; componente Blu				
		MpushReal4 0.0				; componente Verde
		MpushReal4 0.0				; componente Rosso
		call	glColor3f			; usa colore per disegnare
		...
		MpushReal4 -1.0
		MpushReal4 1.0
		call	glVertex3f			; In basso a destra

	invoke	glEnd					; Ho finito di disegnare il triangolo

	;glTranslatef(3.0f, 0.0f, 0.0f);
	MpushReal4 0.0
	MpushReal4 0.0
	MpushReal4 3.0
	call	glTranslatef				; Spostati di 3 caselle a destra			

	MpushReal4 1.0					; componente Blu				
	MpushReal4 0.5					; componente Verde
	MpushReal4 0.5					; componente Rosso
	call	glColor3f				; usa colore per disegnare


	invoke	glBegin, GL_QUADS			; Disegna un quadrilatero

		MpushReal4 0.0
		MpushReal4 1.0
		MpushReal4 -1.0
		call	glVertex3f			; In alto a sinistra

		MpushReal4 0.0
		MpushReal4 1.0
		MpushReal4 1.0
		call	glVertex3f			; In alto a destra
	...
	invoke	glEnd					; Ho finito di disegnare il quadrilatero
